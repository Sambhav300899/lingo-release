...
class LingoDataset(Dataset):
        ...
        **kwargs,
        ...
        if self.load_language:
            if self.max_window_size == 16:
                language_motion_dict_filename = (
                    "language_motion_dict__inter_and_loco__16.pkl"
                )

            with open(
                os.path.join(
                    self.folder, "language_motion_dict", language_motion_dict_filename
                ),
                "rb",
            ) as f:
        ...
        if self.load_scene:
            ...
            if train:
                ...
                scene_file_list = [
                    ...
                    if file.split(".")[0] == self.test_scene_name
                ]
            ...
            # Determine grid size dynamically based on scene_occ bounds
            # For 300x100x400 grid
            if self.scene_occ.shape[1] == 300:
                self.scene_grid_np = np.array([-3, 0, -4, 3, 2, 4, 300, 100, 400])
                self.scene_grid_torch = torch.tensor(
                    [-3, 0, -4, 3, 2, 4, 300, 100, 400]
                ).to(device)
            # For 400x100x600 grid
            elif self.scene_occ.shape[1] == 400:
                self.scene_grid_np = np.array([-4, 0, -6, 4, 2, 6, 400, 100, 600])
                self.scene_grid_torch = torch.tensor(
                    [-4, 0, -6, 4, 2, 6, 400, 100, 600]
                ).to(device)
            else:
                self.scene_grid_np = np.array(
                    [
                        -3,
                        0,
                        -4,
                        ...
                        2,
                        4,
                        self.scene_occ.shape[1],
                        self.scene_occ.shape[2],
                        self.scene_occ.shape[3],
                    ]
                )
                self.scene_grid_torch = torch.tensor(
                    [
            ...
            self.batch_id = (
                torch.linspace(0, batch_size - 1, batch_size)
                .tile((nb_voxels[0] * nb_voxels[1] * nb_voxels[2], 1))
                .T.reshape(-1, 1)
    ...
    def __getitem__(self, idx):
        if self.load_language:
            ...
            text_clip_embedding = self.clip_features[
                [self.text2features_idx[text]]
            ]  # (1, 768)
            text_clip_embedding = torch.from_numpy(text_clip_embedding).float()
            text_clip_embedding = text_clip_embedding / torch.norm(
                text_clip_embedding, dim=1, keepdim=True
            )
            ...
            if left_hand_inter_frame != -1:
                hand_goal = self.joints[
                    left_hand_inter_frame, 24
                ].copy()  # left hand index1
                is_pick = np.ones((1,)).astype(bool)
            elif right_hand_inter_frame != -1:
                hand_goal = self.joints[
                    right_hand_inter_frame, 26
                ].copy()  # right hand index1
                is_pick = np.ones((1,)).astype(bool)
        ...
        init_joints = np.array([joints[0, 0, 0], 0.0, joints[0, 0, 2]])

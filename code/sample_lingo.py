...
def sample_step(cfg, step, mat, fixed_points, sampler, cond, trajectory, pi):
    ...
    pelvis_goal = transform_points(
        pelvis_goal.reshape(1, 1, 3), torch.inverse(mat)
    ...
    if is_loco:
        ...
        pelvis_goal = (
            torch.tensor(
                [
                    trajectory[min(min_idx + 50, len(trajectory) - 1)][0],
                    0,
                    trajectory[min(min_idx + 50, len(trajectory) - 1)][1],
                ]
            )
            .reshape(1, 1, 3)
            .to(cfg.device)
        ...
        for theta in theta_list:
            ...
            goal_points[:, 1] += (
                torch.arange(0.1, 1, 0.1).to(cfg.device).reshape(-1)
            )  # 9x3
            goal_points = goal_points.squeeze(0).repeat(20, 1, 1)
            goal_points[:, :, [0, 2]] *= (
                torch.arange(0, 1, 0.05).to(cfg.device).reshape(-1, 1, 1)
            )

            goal_points = transform_points(
                goal_points, mat
    ...
    samples, occs = sampler.p_sample_loop(
        fixed_points,
        mat,
        scene_flag,
        text_emb,
    ...
    info_dict = {
        "points_orig": points_orig.reshape(
            cfg.batch_size, cfg.max_window_size, 3 * cfg.dataset.nb_joints
        ),
        "pelvis_goal": transform_points(pelvis_goal, mat).reshape(
            cfg.batch_size, 3
        ),  # global coordinates
        "pi": pi,
        "need_pi": need_pi,
...
def get_mat(cfg, points):
    pelvis_new = (
        points[:, -cfg.auto_regre_num, :9].cpu().numpy().reshape(cfg.batch_size, 3, 3)
    )
...
def get_guidance(cfg, seg_id):
    ...
    cond["text_emb"] = (
        get_clip_features(input["text"]).reshape(cfg.batch_size, 1, -1).to(cfg.device)
    )
    cond["pelvis_goal"] = input["end_location"]
    cond["pelvis_goal"] = np.array(
        [cond["pelvis_goal"][0], 0, cond["pelvis_goal"][2]]
    )  # set y to 0
    cond["pelvis_goal"] = torch.from_numpy(cond["pelvis_goal"].astype(np.float32)).to(
        cfg.device
    )
    cond["hand_goal"] = (
        torch.from_numpy(input["hand_location"].astype(np.float32))
        .to(cfg.device)
        .reshape(cfg.batch_size, 1, 3)
    )

    if "pick up" in input["text"] or "put down" in input["text"]:
        cond["is_pick"] = torch.ones((cfg.batch_size, 1), dtype=torch.bool).to(
            cfg.device
        )
    else:
        cond["is_pick"] = torch.zeros((cfg.batch_size, 1), dtype=torch.bool).to(
            cfg.device
        )

    if (
        "walk" in cond["raw_text"]
        or "sit down" in cond["raw_text"]
        or "lie down" in cond["raw_text"]
        or "type on " in cond["raw_text"]
        or "write on" in cond["raw_text"]
        or "wash " in cond["raw_text"]
        or "punch " in cond["raw_text"]
        or "kick " in cond["raw_text"]
    ):
    ...
    if (
        ...
        need_pelvis_dir = torch.zeros((cfg.batch_size,), dtype=torch.bool).to(
            cfg.device
        )
    ...
    cond["is_loco"] = (
        torch.ones((cfg.batch_size,), dtype=torch.bool).to(cfg.device)
        if "walk" in cond["raw_text"]
        else torch.zeros((cfg.batch_size,), dtype=torch.bool).to(cfg.device)
    )
    cond["need_pi"] = (
        torch.zeros((cfg.batch_size,), dtype=torch.bool).to(cfg.device)
        if "walk" in cond["raw_text"]
        else torch.ones((cfg.batch_size,), dtype=torch.bool).to(cfg.device)
    )

    cond["start_location"] = input["start_location"]
    cond["start_location"] = np.array(
        [cond["start_location"][0], 0, cond["start_location"][2]]
    )
...
def sample(cfg: DictConfig) -> None:
    ...
    print(
        "model_joints_to_smplx device: ",
        next(model_joints_to_smplx.parameters()).device,
    )
    ...
    if cfg.use_scheduler:
        ...
        scheduler_model.load_state_dict(
            torch.load(cfg.scheduler_model_path, map_location=device)
        )
    ...
    for seg_id in range(seg_num):
        ...
        if seg_id == 0:
            ...
            joints, mat, _, _, _, _, _, _, _, _, _, _ = (
                sampler_body.dataset.__getitem__(stand_start_idx_list[0])
            )
            joints = (
                torch.from_numpy(joints)
                .float()
                .reshape(1, -1, cfg.dataset.nb_joints * 3)
            )
            mat = torch.from_numpy(mat).float().reshape(1, 4, 4)

            points, mat = joints.to(device), mat.to(device)
            points_orig = sampler_body.dataset.denormalize_torch(
                points
            )  # (batch_size, max_window_size, nb_joints*3)

            theta = (
                np.arctan2(
                    -cond["pelvis_goal"].cpu().numpy()[2]
                    + cond["start_location"].cpu().numpy()[2],
                    cond["pelvis_goal"].cpu().numpy()[0]
                    - cond["start_location"].cpu().numpy()[0],
                )
                + np.pi / 2
            )
            ...
            points_orig = (
                points_orig.reshape(
                    cfg.batch_size, cfg.max_window_size, cfg.dataset.nb_joints, 3
                )
                @ mat[0, :3, :3].t()
            )
            points_orig = points_orig.reshape(
            ...
            translation_shift = (
                points_orig[:, [-cfg.auto_regre_num], :3] - cond["start_location"]
            )
            translation_shift[0, 0, 1] = 0.0
            points_orig = points_orig.reshape(
                cfg.batch_size, -1, cfg.dataset.nb_joints, 3
            )
            points_orig[:, :, :] -= translation_shift
            points_orig = points_orig.reshape(
                cfg.batch_size, -1, 3 * cfg.dataset.nb_joints
            )
        else:
            points_orig = torch.from_numpy(
                points_all[-1].reshape(cfg.batch_size, -1, cfg.dataset.nb_joints * 3)
            ).to(cfg.device)
        ...
        if cond["is_loco"]:
            if seg_id == 0:
                ...
                start_loc = (
                    points_orig[:, -cfg.auto_regre_num]
                    .reshape(cfg.batch_size, cfg.dataset.nb_joints, 3)[0, 0, [0, 2]]
                    .cpu()
                    .numpy()
                )
        ...
        for step in tqdm(range(seg_len)):
            if step == 0:
                ...
                fixed_points = points_orig[:, -cfg.auto_regre_num :].reshape(
                    cfg.batch_size, cfg.auto_regre_num, cfg.dataset.nb_joints * 3
                )
                fixed_points = sampler_body.dataset.normalize_torch(
                    transform_points(fixed_points, torch.inverse(mat))
                )
            else:
                mat = get_mat(cfg, points)
                fixed_points = points[:, -cfg.auto_regre_num :].reshape(
                    cfg.batch_size, cfg.auto_regre_num, cfg.dataset.nb_joints * 3
                )
                fixed_points = sampler_body.dataset.normalize_torch(
                    transform_points(fixed_points, torch.inverse(mat))
            ...
            pi = torch.tensor(
                [
                    int(
                        (step + phase)
                        * (cfg.max_window_size - cfg.auto_regre_num)
                        * speed_inter
                    )
                ]
            ...
            info_dict = sample_step(
                cfg, step, mat, fixed_points, sampler_body, cond, trajectory, pi
            )
            points = info_dict[
                "points_orig"
            ]  # points in global coordinates and is denormalized
            ...
            if step == seg_len - 1:
                ...
                if step == 0 and seg_id > 0:
                    points_all.append(
                        points.cpu().numpy()[
                            :, cfg.auto_regre_num : -cfg.auto_regre_num
                        ]
                    )
                else:
                    points_all.append(points.cpu().numpy()[:, : -cfg.auto_regre_num])
            ...
            if cfg.use_scheduler and not cond["is_loco"]:
                points_loco = sampler_body.dataset.normalize_torch(
                    transform_points(points, torch.inverse(mat))
                )
                stop_pred = scheduler_model(points_loco, cond["text_emb"], pi).squeeze(
                    1
                )
            ...
            if cond["is_loco"] and seg_id != seg_num - 1:
                ...
                dist2end = np.linalg.norm(
                    curr_loc - end_point
                )  # distance to the end point
                if dist2end < cfg.locomotion_threshold:
                    break

    points_all = np.concatenate(points_all, axis=1).reshape(
        cfg.batch_size, -1, cfg.dataset.nb_joints, 3
    )
    ...
    for i in range(cfg.batch_size):
        ...
        pose, transl, _, _ = joints_to_smpl(
